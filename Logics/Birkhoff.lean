...
theorem ptOf_injective (antisymm : ∀ x y : P, x ≼ y → y ≼ x → x = y) :
...
/-- The upward closed sets generated by the entries of `l` lying in `U`. -/
noncomputable def generators (l : List P) (U : Upset P) : List (Upset P) :=
  (l.filter fun x => truth (U.mem x)).map Upset.up

theorem up_mem_generators {l : List P} (hl : ∀ x, x ∈ l) {U : Upset P} {x : P}
    (hx : U.mem x) : Upset.up x ∈ generators l U :=
  List.mem_map.mpr ⟨x, List.mem_filter.mpr ⟨hl x, truth_eq_true.mpr hx⟩, rfl⟩

theorem mem_of_generators {l : List P} {U V : Upset P} (hV : V ∈ generators l U) :
    ∃ x, U.mem x ∧ V = Upset.up x := by
...
theorem ptOf_surjective {l : List P} (hl : ∀ x, x ∈ l) :
  ...
  intro j
  have hle : j.val ⊑ supList (generators l j.val) := fun x hx =>
    (le_supList (up_mem_generators hl hx) : Upset.up x ⊑ supList (generators l j.val))
      x (Frame.le_refl x)
  obtain ⟨U, hUL, hjU⟩ := j.irred.le_supList hle
  obtain ⟨x, hxj, rfl⟩ := mem_of_generators hUL
...
theorem birkhoff_poset (antisymm : ∀ x y : P, x ≼ y → y ≼ x → x = y)
    {l : List P} (hl : ∀ x, x ∈ l) :
